...
import { RegisteredModuleKeys } from '~/lib/constants';

export const loader = async ({ params }: LoaderFunctionArgs) => {
  invariant(params.serverId, 'Missing serverId param');
  const guildConfig = await getAllConfigs(params.serverId);
  ...
  const moduleConfigs = Object.fromEntries(
    Object.entries(guildConfig).filter(([key]) =>
      RegisteredModuleKeys.includes(key)
    )
  );
  return json({ guildConfig, moduleConfigs });
};
...
const SERVER_MANAGEMENT_NAV = [
  { key: 'welcomeConfig', to: './welcome', label: 'Welcome', weight: 1 },
  { key: 'leaveConfig', to: './leave', label: 'Goodbye', weight: 2 },
  { key: 'autoroleConfig', to: './auto_roles', label: 'Auto Roles', weight: 3 },
  { key: 'moderationConfig', to: './moderator', label: 'Moderator', weight: 4 },
];
            ...
            <ul role="list" className="flex flex-1 flex-col gap-y-7">
              ...
              <li>
                ...
                <ul role="list" className="-m-2 mt-2">
                  {SERVER_MANAGEMENT_NAV.map(({ label, to }, i) => (
                    <li key={i}>
                      <NavLink
                        to={to}
                        className={({ isActive, isPending }) =>
